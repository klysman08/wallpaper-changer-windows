...
- [ ] 10.3 **Decode a batch across cores**, with a bounded thread count rather than one per image. Python was serial too, so this is not a regression — but it is the difference between 1.7x slower than Pillow and 4.6x faster. **Still outstanding, and now the binding constraint on the picker**: with the resampler fixed, `get_thumbnails` is dominated by decoding, and 120 thumbnails take 3.8 s against Pillow's ~2.5 s. The resampler swap did not help here because shrinking to 160 px was never the expensive part.
