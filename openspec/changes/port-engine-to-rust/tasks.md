...
- [x] 10.2 **Move the image methods off the async workers — done.** `get_thumbnails`, `get_image_preview`, **`preview` and `save_collage`** now run on `spawn_blocking`. The plan named only the first two; the other two compose images and had exactly the same problem, so all four moved. While a folder loaded, every other call queued behind these — the "whole app feels heavy" half of the report, distinct from raw throughput. `blocking` moved from `session.rs` to the crate root now that two modules use it, and it doubles as the panic guard that the synchronous arms get from `guard`.
- [x] 10.3 **Decode a batch across cores — done.** With the resampler fixed, `get_thumbnails` was dominated by decoding, and the resampler swap had not helped there because shrinking to 160 px was never the expensive part. **120 thumbnails went from 3.82 s to 1.16 s**, past Pillow's ~2.5 s for the same batch.
  The scaffolding is now shared: `parallel::map_bounded` runs a list of independent jobs across at most four threads and **returns results in input order**, which is what lets `compose_collage` still report the first broken picture in plan order rather than whichever thread lost first. `fit_all` was refactored onto it, so there is one piece of threading code rather than two. A panicking job is reported instead of leaving a hole in the results — except in `get_thumbnails`, which answers with what it has, because its contract is that one bad picture costs its own tile and nothing else.
  Four workers, not the core count, and for the same reason as the fit: each holds a decoded full-size image, sixty megabytes for a large photograph, and memory was half the original report.
- [x] 10.4 **Stop loading libmpv at start-up — done.** `App.tsx` calls `getCapabilities()` on launch, which called `has_mpv()`, which mapped the **112 MB** DLL every launch even when video was never used; under Python that cost at least landed in the sidecar. `has_mpv` now answers the question its callers actually mean — is there a libmpv to load — from the file's presence, and reports authoritatively if one is already loaded. The real load moved to `start_inputs`, at the moment playback is asked for, so a DLL that exists but will not load still reports `no_mpv` rather than a bare error. `get_capabilities` answers `has_mpv: true` in 0.171 s including process start-up, and a test asserts the probe does not *cause* a load.
...
- [ ] 10.7 **Preview and apply are 2.3x slower than Python, and it is the resampler.** Measured on the reporter's folder and monitors, five previews in one process at `max_width` 960: **Rust release 2.31 s each, Python 1.0 s each.** Decoding is only about 6% of that (four images at ~36 ms), so the cost is `imageops::resize` with Lanczos3, which is scalar Rust where Pillow's is SIMD. Three ways out, in increasing order of risk:
  - [x] **Parallelise the per-cell fit — done, and it closed the gap.** `compose_collage` now gathers the distinct (picture, cell size) pairs, fits them across at most four threads, and pastes serially. **5 previews went from 11.56 s to 5.36 s: 2.31 s each to 1.07 s, level with Python's ~1.0 s.** All 180 unit tests and all 3 golden images still pass, which is the point — this is scheduling, not arithmetic, so not a pixel moved. The worker cap is deliberately 4 rather than the core count: each worker holds a decoded source *and* its fitted output, and memory was half the original complaint. Failures are reported in plan order rather than whichever thread lost first, so a broken folder gives the same message every time.
  - [x] **Swap the resampler for `fast_image_resize` — done.** SIMD Lanczos3 behind one `images::resize_lanczos3`, which composition, thumbnails and the preview downscale all now share so they cannot drift apart. **Preview went from 1.07 s to 0.336 s**, and the whole journey is 2.31 s -> 1.07 s -> 0.336 s against Python's ~1.0 s: three times quicker than what it replaced. The unit suite dropped from 167 s to 47 s as a side effect.
    **The goldens were not regenerated, and must not be.** They are Pillow's output, frozen while both engines existed; rebuilding them from Rust would leave them comparing the port against itself, and the Python composer is gone so there is no second chance to make them. What moved is the *bound*, re-derived from a new `fit_drift_against_the_goldens` reporter rather than by loosening until the failure stopped: pure downscales 1 -> **2**, the two `fit` downscales unchanged at 1, upscales still inside the existing 24 (worst 22), and a same-size resize now **exactly 0** where it had been allowed 1 — so that case was *tightened*. Two levels out of 255 is below anything visible in a wallpaper.
    Worth recording that the expectation going in was wrong: `fast_image_resize` implements the same normalised separable convolution as Pillow, so it looked likely to land *closer* to the goldens. On pure downscales it landed slightly further away. The measurement is the reason the bound is 2 and not a guess.
  - **Compose the preview at preview scale** instead of composing the full virtual desktop (5760x2160 here) and downscaling to 960. About 36x less resampling, but the preview would no longer be pixel-identical to what apply produces, which is the point of a preview.
  Note that **the compose-then-downscale shape is inherited**: `rpc.py` did exactly the same, so the port did not make this architecturally worse — only slower per pixel.
- [ ] 10.8 Verify against the numbers above, on the same folder, and record them.

## 11. Format robustness (1-2 days)

**Raised by testing, and the cause is found.** The reported "Error parsing image / Illegal start bytes: 5249" was real, and `5249` was the whole clue: `0x52 0x49` is `RI`, the front of a **RIFF** header. A WebP was being handed to the JPEG decoder.

`image::open` documents that it picks the decoder "from the path's file extension". Composing used it; thumbnails used a content-sniffing loader. So the *same picture* produced a working thumbnail and a failed preview — which is exactly what "the picker looks fine but preview and apply are broken" means. `compose_collage` propagates with `?`, so one such file failed the entire composition rather than itself.
